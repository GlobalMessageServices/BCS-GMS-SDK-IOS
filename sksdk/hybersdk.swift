    ...
    public func sk_register_new(user_phone: String, user_password: String, x_hyber_sesion_id: String, x_hyber_ios_bundle_id: String, x_hyber_app_fingerprint: String)->String {
        do{
            ...
            if (Constants.registrationstatus==false){
                ...
                let sk_rest_server = HyberAPI.init()
                let abbb = sk_rest_server.hyber_device_register(X_Hyber_Client_API_Key: token_fb, X_Hyber_Session_Id: x_hyber_sesion_id, X_Hyber_IOS_Bundle_Id: x_hyber_ios_bundle_id,  device_Name:UIDevice.modelName, device_Type: Constants.localizedModel, os_Type: "ios", sdk_Version: Constants.sdkVersion, user_Pass: user_password, user_Phone: user_phone)
    ...
    public func sk_clear_current_device()->String {
        do{
            if (Constants.registrationstatus==true){
                ...
                let sk_rest_server = HyberAPI.init()
                let ggg = sk_rest_server.sk_device_revoke(dev_list: [Constants.deviceId!], X_Hyber_Session_Id: X_Hyber_Session_Id, X_Hyber_Auth_Token: Constants.hyber_registration_token!)
                return ggg
    ...
    public func sk_get_device_all_from_sk() -> String {
        do{
            if (Constants.registrationstatus==true){
                let X_Hyber_Session_Id: String = Constants.firebase_registration_token ?? "22"
                let sk_rest_server = HyberAPI.init()
                let ansss = sk_rest_server.sk_device_get_all(X_Hyber_Session_Id: X_Hyber_Session_Id, X_Hyber_Auth_Token: Constants.hyber_registration_token!)
                print(ansss)
    ...
    public func sk_send_message_callback(message_id: String, message_text: String) -> String {
        do{
            if (Constants.registrationstatus==true){
                let X_Hyber_Session_Id: String = Constants.firebase_registration_token ?? "22"
                let sk_rest_server = HyberAPI.init()
                let anss = sk_rest_server.sk_message_callback(message_Id: message_id, answer: message_text, X_Hyber_Session_Id: X_Hyber_Session_Id, X_Hyber_Auth_Token: Constants.hyber_registration_token!)
    ...
    public func sk_message_delivery_report(message_id: String) -> String {
        do{
            if (Constants.registrationstatus==true){
                ...
                let asaa = sk_rest_server.sk_message_dr(message_Id: message_id, received_At: "123123122341", X_Hyber_Session_Id: X_Hyber_Session_Id, X_Hyber_Auth_Token: Constants.hyber_registration_token!)
    ...
    public func sk_get_message_history(period_in_seconds: Int) -> String {
        do {
            if (Constants.registrationstatus==true){
                ...
                let ansss = sk_rest_server.sk_message_get_history(utc_time: period_in_seconds, X_Hyber_Session_Id: X_Hyber_Session_Id, X_Hyber_Auth_Token: Constants.hyber_registration_token!)
                return ansss}
            else {
                return answer_b.general_answer(resp_code: "704", body_json: "error", description: "Not registered")
            }
    ...
    public func sk_clear_all_device()->String {
        do{
            ...
            if (Constants.registrationstatus==true){
                ...
                let sk_rest_server = HyberAPI.init()
                let ggg = sk_rest_server.sk_device_revoke(dev_list: listdev, X_Hyber_Session_Id: X_Hyber_Session_Id, X_Hyber_Auth_Token: Constants.hyber_registration_token!)
